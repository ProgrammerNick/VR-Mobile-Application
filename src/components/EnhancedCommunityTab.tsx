...
const EnhancedCommunityTab = () => {
  ...
  const [socialPosts, setSocialPosts] = useState<CommunityPost[]>([
    {
      ...
      timestamp: '2h ago',
      content: "Just reached the final boss in 'Galaxy Raiders'! This game is amazing. The starship battles are intense!",
      game: {
        name: 'Galaxy Raiders',
        imageUrl: 'https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=300&h=200&fit=crop',
      },
      likes: 12,
      comments: 4,
    },
    {
      ...
      timestamp: '5h ago',
      content: "Looking for a team for the 'Echo Arena' weekly tournament. Anyone interested? I'm a decent goalie.",
      game: {
        name: 'Echo Arena',
        imageUrl: 'https://images.unsplash.com/photo-1551712709-a64b0717f09d?w=300&h=200&fit=crop',
      },
      likes: 8,
      comments: 15,
    },
    {
      ...
      timestamp: '1d ago',
      content: "Spent the whole afternoon in 'Tilt Brush' creating a 3D masterpiece. The creative possibilities are endless in VR.",
      likes: 25,
      comments: 7,
    },
  ]);
    ...
    setEvents([
      {
        ...
        description: "Join us for our weekly Beat Saber competition! Open to all skill levels.",
      ...
      },
      {
        ...
        description: "Learn to create amazing worlds in Horizon Call of the Mountain",
        game_name: "Horizon",
    ...
    setLfgPosts([
      {
        ...
        post_text: "LF2M for ranked matches tonight. Prefer experienced players!",
        slots_needed: 2,
        slots_filled: 0
      },
      {
        ...
        author_id: "user2",
        game_name: "Rec Room",
        post_text: "Starting a new crew! Looking for creative builders to join our group.",
        slots_needed: 5,
        slots_filled: 2
      }
    ]);
    ...
    setAchievements([
      {
        ...
        screenshot_url: "https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=300&h=200&fit=crop",
        likes_count: 24,
        profiles: {
  ...
  const handleCreateAchievement = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
  ...
  return (
    <div className="p-4 space-y-6 h-full overflow-y-auto pb-24">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-700 rounded-xl p-4 text-black">
        ...
        <p className="text-black/80 mt-1">Connect, compete, and create with fellow VR enthusiasts</p>
      </div>
              ...
              <div className="p-4">
                ...
                {post.game && (
                  <div className="mt-4 p-3 bg-muted/50 rounded-lg flex items-center gap-4">
                    <img src={post.game.imageUrl} alt={post.game.name} className="w-16 h-16 object-cover rounded-md" />
                    <div className="flex-grow">
                      <p className="font-semibold text-sm">{post.game.name}</p>
                      <Button variant="outline" size="sm" className="mt-1">View Game</Button>
                    </div>
                  </div>
                )}
                <div className="flex items-center justify-between mt-4 text-muted-foreground">
                  <div className="flex items-center space-x-4">
                    ...
                    >
                      <ThumbsUp className={`w-4 h-4 ${userLikes.includes(parseInt(post.id)) ? "fill-current" : ""}`} />
              ...
              {events.map((event) => (
                <Card key={event.id} className="overflow-hidden">
                  <div className="p-4">
                    ...
                    <Button className="w-full mt-4" onClick={() => handleJoinEvent(event.id)}>
                      Join Event
                    </Button>
                  </div>
                </Card>
              ...
              {challenges.map((challenge) => (
                <Card key={challenge.id} className="overflow-hidden">
                  <div className="p-4">
                    ...
                    <Button className="w-full mt-4" onClick={() => handleJoinChallenge(challenge.id)}>
                      Join Challenge
                    </Button>
                  </div>
                </Card>
      ...
      <Card>
          ...
          <Dialog open={isCreateAchievementOpen} onOpenChange={setIsCreateAchievementOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Share something you're proud of with the community.</DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateAchievement} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="achievement_title">Achievement</Label>
                  <Input id="achievement_title" name="achievement_title" placeholder='e.g., "Reached Master Rank"' required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="game_name">Game</Label>
                  <Input id="game_name" name="game_name" placeholder="e.g., Beat Saber" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="description">Description (Optional)</Label>
                  <Textarea id="description" name="description" placeholder="A few words about how you did it..." />
                </div>
                <div className="space-y-2">
                ...
                <DialogFooter>
                  <Button type="button" variant="ghost" onClick={() => setIsCreateAchievementOpen(false)}>Cancel</Button>
        ...
        <CardContent>
          {loadingAchievements ? <p>Loading achievements...</p> : achievements.length === 0 ? (
            <p className="text-muted-foreground text-sm">No achievements posted yet. Be the first!</p>
          ) : (
            <div className="space-y-4">
              {achievements.map((post) => (
                <Card key={post.id} className="overflow-hidden">
                  {post.screenshot_url && <img src={post.screenshot_url} alt={post.achievement_title} className="w-full h-32 object-cover" />}
                  <div className="p-4">
                    <div className="flex items-start space-x-4">
                      <Avatar>
                      ...
                      <div className="flex-grow">
                        ...
                        <h4 className="text-lg font-bold tracking-tight">{post.achievement_title}</h4>
                        {post.description && <p className="text-sm text-muted-foreground mt-1">{post.description}</p>}
                        <Badge variant="secondary" className="mt-2">{post.game_name}</Badge>
                      </div>
                    </div>
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center space-x-2">
                        <Button 
                          variant={userLikes.includes(post.id) ? "default" : "ghost"}
                          size="sm" 
      ...
      <Dialog open={isCreatePostOpen} onOpenChange={setIsCreatePostOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Share your thoughts, experiences, or questions with the community.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreatePost} className="space-y-4">
            ...
            <DialogFooter>
              <Button type="button" variant="ghost" onClick={() => setIsCreatePostOpen(false)}>Cancel</Button>
              <Button type="submit">Post</Button>
            </DialogFooter>
          </form>
        </DialogContent>
